...
class MenuNode: SKSpriteNode, ResizableNode, SoundDelegate {
    ...
    init(parentSize size: CGSize) {
        ...
        highScore = defaults.integer(forKey: PreferencesKeys.highscoreKey)
    ...
    func showScreen(parentSize: CGSize) {
        self.alpha = 0
        let fadeIn = SKAction.fadeIn(withDuration: UIGlobals.gameSpeed)
        self.run(fadeIn)
    ...
    func relayout(parentSize size: CGSize) {
        ...
        if size.width > 414 {
            ...
            let labelWidth = min(UIHelpers.maxLabelWidth, size.width - 2 * UIHelpers.margin)
            resetHighScoreLabel.adjustsFontSizeToFitWidth(CGRect(x: UIHelpers.margin, y: downMargin + row_height + UIHelpers.margin, width: labelWidth, height: row_height), centered: true)
            let calculatedFontSize = min(UIHelpers.maxFontSize, resetHighScoreLabel.fontSize)
            setupLabelTexts()
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        case resetHighScoreLabel:
            ...
            gameScoreDelegate?.setHighScore(highScore: 0)
            setupLabelTexts()
        default:
            break
        }
