...
final class Board: NSObject, NSCoding {
    ...
    init(dimension: Int) {
        ...
        self.board = [[Gem]]()
            ...
            var arr = [Ice]()
                for j in o < dimension {
                arr.append(Ice(type: IceType.None))
            }
            board.append(arr)
        }
    }
    ...
    init(copyFromBoard other: Board) {
        ...
        self.board = [[Gem]]() //change to Ice
            for i in 0 < dimension{
            var arr = [Gem]()
                for j in 0 < dimension {
                arr.append(Ice(type: other.board[i][j].type))
            }
            board.append(arr)
        }
    }
    ...
    private func randomType(fromSet set: Set<GemType>) -> GemType {
        ...
        for (count, item) in (set.sort() as AnyObject).enumerated() {
            if count == dice {
                return item
            }
        }
    ...
    private func clearGemsInSet(set: Set<ExplodingGem>) {
        for index in set {
            clearGemAt(index: index.index)
        }
    }
    
    private func swapGems(gem1: GemIndex, gem2: GemIndex) {
    ...
    private func randomGem() -> GemType {
        return randomGems(count: 1).first!
    }
    ...
    /**
        Returns a hint.
    */
    func hint( index1: inout GemIndex, index2: inout GemIndex) -> Bool {
    ...
    */
    ...
    func gatherSimpleCollisions() -> Set<ExplodingGem> {
        ...
        for i in 2..<(dimension + 2) {
            ...
            for j in 2..<(dimension + 2) {
                if board[i][j].type == currentType {
                    currentSet.nonUpdatingInsert(member: ExplodingGem(index: GemIndex(x: i, y: j), type:currentType, cause: .Normal))
                    continue
                }
                if currentSet.count > 2 {
                    ret.nonUpdatingUnionInPlace(sequence: currentSet)
                    b.clearGemsInSet(set: currentSet)
                }
        ...
        self.board = b.board
        createSuperGemsFromSet(explosions: ret)
        return ret
    }
    
    private func createSuperGemsFromSet( explosions: Set<ExplodingGem>) {
        while explosions.count > 0 {
            let explosion = explosions.first!
            
            let horizontalSet = horizontalExpansionFromSet(set: explosions, at: explosion)
            var verticalExpansions = Array<Set<ExplodingGem>>()
            for hExplosion in horizontalSet {
                verticalExpansions.append(verticalExpansionFromSet(set: explosions, at: hExplosion))
            }
            verticalExpansions.sortInPlace({$0.count > $1.count})
            
            let verticalSet = verticalExpansionFromSet(set: explosions, at: explosion)
            var horizontalExpansions = Array<Set<ExplodingGem>>()
            for vExplosion in verticalSet {
                horizontalExpansions.append(horizontalExpansionFromSet(set: explosions, at: vExplosion))
            }
    ...
    private func horizontalExpansionFromSet(set: Set<ExplodingGem>, at: ExplodingGem) -> Set<ExplodingGem> {
        ...
        for i in 0..<dimension {
            if !addToSet(set: &horizontalSet, fromSet: set, ifItContains: GemIndex(x: at.index.x + i, y: at.index.y), withType: at.type) {
                break
            }
        }
        for i in 0..<dimension {
    ...
    private func verticalExpansionFromSet(set: Set<ExplodingGem>, at: ExplodingGem) -> Set<ExplodingGem> {
        ...
        for j in 0..<dimension {
            if !addToSet(set: &verticalSet, fromSet: set, ifItContains: GemIndex(x: at.index.x, y: at.index.y + j), withType: at.type) {
                break
            }
        }
        for j in 0..<dimension {
    ...
    private func addToSet( set: inout Set<ExplodingGem>, fromSet: Set<ExplodingGem>, ifItContains index: GemIndex, withType type: GemType) -> Bool {
    ...
    func checkSwap(forGemIndex: GemIndex, withGemIndex: GemIndex) -> Set<ExplodingGem> {
    ...
    private func checkSwapFor(superGemBomb superGem: GemIndex, withGem: GemIndex) -> Set<ExplodingGem> {
        ...
        ret.nonUpdatingInsert(member: ExplodingGem(index: superGem, type: .SuperGemBomb, cause: .SuperGemBomb(index: superGem), processed: true))
        clearGemAt(index: superGem)
        ...
        for i in iMin...iMax {
            ...
            for j in jMin...jMax {
                ...
                ret.nonUpdatingInsert(member: ExplodingGem(index: currentIndex, type: board[currentIndex.x][currentIndex.y].type, cause: .SuperGemBomb(index: superGem)))
                clearGemAt(index: currentIndex)
            }
        }
        return ret
    }
    ...
    private func checkSwapFor(superGemColor superGem: GemIndex, withGem: GemIndex) -> Set<ExplodingGem> {
        ...
        var ret = gatherSimpleCollisions()
        ret.nonUpdatingInsert(member: ExplodingGem(index: superGem, type: .SuperGemColor, cause: .SuperGemColor(index: superGem), processed: true))
        clearGemAt(index: superGem)
    ...
    private func checkSwapFor(superGemLine superGem: GemIndex, withGem: GemIndex) -> Set<ExplodingGem> {
        ...
        var ret = gatherSimpleCollisions()
        ret.nonUpdatingInsert(member: ExplodingGem(index: superGem, type: .SuperGemLine, cause: .SuperGemLine(index: superGem), processed: true))
        clearGemAt(index: superGem)
        // horizontal sweep
        for i in 2..<(dimension + 2) {
            let index = GemIndex(x: i, y:superGem.y)
            ret.nonUpdatingInsert(member: ExplodingGem(index: index, type: board[i][superGem.y].type, cause: .SuperGemLine(index: superGem)))
            clearGemAt(index: index)
        }
        // vertical sweep
        for j in 2..<(dimension + 2) {
            let index = GemIndex(x: superGem.x, y:j)
            ret.nonUpdatingInsert(member: ExplodingGem(index: index, type: board[superGem.x][j].type, cause: .SuperGemLine(index: superGem)))
            clearGemAt(index: index)
        }
        return ret
    }
    
    private func explosionsForSwap(bomb1: GemIndex, bomb2: GemIndex) -> Set<ExplodingGem> {
        var ret = Set<ExplodingGem>()
        ...
        clearGemAt(index: bomb1)
        clearGemAt(index: bomb2)
        ...
        for i in iMin...iMax {
            ...
            for j in jMin...jMax {
                let currentIndex = GemIndex(x: i, y: j)
                ret.nonUpdatingInsert(member: ExplodingGem(index: currentIndex, type: board[i][j].type, cause: .SuperGemBomb(index: bomb2)))
                clearGemAt(index: currentIndex)
            }
        }
        return ret
    ...
    private func explosionsForSwap(bomb: GemIndex, color: GemIndex) -> Set<ExplodingGem> {
        var ret = Set<ExplodingGem>()
        ret.nonUpdatingInsert(member: ExplodingGem(index: bomb, type: .SuperGemBomb, cause: .SuperGemColor(index: color), processed: true))
        ret.nonUpdatingInsert(member: ExplodingGem(index: color, type: .SuperGemColor, cause: .SuperGemBomb(index: bomb), processed: true))
        clearGemAt(index: bomb)
        clearGemAt(index: color)
        let type = randomType(fromSet: normalGemTypesOnBoard(excludedIndexes: Set<GemIndex>()))
        let indexes = collectGemIndexes(ofType: type)
        for index in indexes {
            ret.nonUpdatingInsert(member: ExplodingGem(index: index, type: .SuperGemBomb, cause: .SuperGemColor(index: color)))
            clearGemAt(index: index)
        }
        return ret
    }
    
    private func explosionsForSwap(bomb: GemIndex, line: GemIndex) -> Set<ExplodingGem> {
        var ret = Set<ExplodingGem>()
        ret.nonUpdatingInsert(member: ExplodingGem(index: bomb, type: .SuperGemBomb, cause: .SuperGemLine(index: line), processed: true))
        ret.nonUpdatingInsert(member: ExplodingGem(index: line, type: .SuperGemLine, cause: .SuperGemBomb(index: bomb), processed: true))
        clearGemAt(index: bomb)
        clearGemAt(index: line)
        // vertical sweep
        ...
        for i in minI...maxI {
            for j in 2..<(dimension + 2) {
                let currentIndex = GemIndex(x: i, y: j)
                ret.nonUpdatingInsert(member: ExplodingGem(index: currentIndex, type: board[currentIndex.x][currentIndex.y].type, cause: .SuperGemLine(index: line)))
                clearGemAt(index: currentIndex)
            }
        }
        // horizontal sweep
    ...
    private func explosionsForSwap(color1: GemIndex, color2: GemIndex) -> Set<ExplodingGem> {
        var ret = Set<ExplodingGem>()
        for i in 2..<(dimension + 2) {
            for j in 2..<(dimension + 2) {
                let index = GemIndex(x: i, y: j)
                ret.nonUpdatingInsert(member: ExplodingGem(index: index, type: board[i][j].type, cause: .SuperGemColor(index: color1), processed: true))
                clearGemAt(index: index)
            }
        }
        return ret
    ...
    private func explosionsForSwap(color: GemIndex, line: GemIndex) -> Set<ExplodingGem> {
        var ret = Set<ExplodingGem>()
        ret.nonUpdatingInsert(member: ExplodingGem(index: line, type: .SuperGemLine, cause: .SuperGemColor(index: color), processed: true))
        ret.nonUpdatingInsert(member: ExplodingGem(index: color, type: .SuperGemColor, cause: .SuperGemLine(index: line), processed: true))
        clearGemAt(index: color)
        clearGemAt(index: line)
        let type = randomType(fromSet: normalGemTypesOnBoard(excludedIndexes: Set<GemIndex>()))
        let indexes = collectGemIndexes(ofType: type)
        for index in indexes {
            ret.nonUpdatingInsert(member: ExplodingGem(index: index, type: .SuperGemLine, cause: .SuperGemColor(index: color)))
            clearGemAt(index: index)
        }
        return ret
    }
    
    private func explosionsForSwap(line1: GemIndex, line2: GemIndex) -> Set<ExplodingGem> {
        var ret = Set<ExplodingGem>()
        ret.nonUpdatingInsert(member: ExplodingGem(index: line1, type: .SuperGemLine, cause: .SuperGemLine(index: line2), processed: true))
        ret.nonUpdatingInsert(member: ExplodingGem(index: line2, type: .SuperGemLine, cause: .SuperGemLine(index: line1), processed: true))
        clearGemAt(index: line1)
        clearGemAt(index: line2)
        // vertical sweep
        // horizontal sweep
        // 2x diagonal sweep
        for i in 2..<(dimension + 2) {
            for j in 2..<(dimension + 2) {
                ...
                if line1.x == i || line1.y == j  || abs((i - j) - (line1.x - line1.y)) == 0 || abs((i + j) - (line1.x + line1.y)) == 0 {
                    ret.nonUpdatingInsert(member: ExplodingGem(index: index, type: board[i][j].type, cause: .SuperGemLine(index: line1)))
                    clearGemAt(index: index)
                    continue
                }
                if line2.x == i || line2.y == j || abs((i - j) - (line2.x - line2.y)) == 0 || abs((i + j) - (line2.x + line2.y)) == 0 {
                    ret.nonUpdatingInsert(member: ExplodingGem(index: index, type: board[i][j].type, cause: .SuperGemLine(index: line2)))
                    clearGemAt(index: index)
                    continue
                }
            }
    ...
    func calculateNextExplosion(withExplosions explosions: Set<ExplodingGem>) -> Set<ExplodingGem> {
        func updateStatusIn( explosions: inout Set<ExplodingGem>, forExplosion explosion: ExplodingGem) {
        ...
        for explosion in explosions.filter({!$0.processed}) {
            ...
            case .SuperGemBomb:
                updateStatusIn(explosions: &ret, forExplosion: explosion)
                ...
                for i in iMin...iMax {
                    ...
                    for j in jMin...jMax {
                        let index = GemIndex(x: i, y: j)
                        ret.nonUpdatingInsert(member: ExplodingGem(index: index, type: board[i][j].type, cause: .SuperGemBomb(index: explosion.index)))
                        clearGemAt(index: index)
                    }
                }
            case .SuperGemColor:
                ...
                let indexes = collectGemIndexes(ofType: type)
                for index in indexes {
                    ret.nonUpdatingInsert(member: ExplodingGem(index: index, type: type, cause: .SuperGemColor(index: explosion.index)))
                    clearGemAt(index: index)
                }
            case .SuperGemLine:
                updateStatusIn(explosions: &ret, forExplosion: explosion)
                for i in 2..<(dimension + 2) {
                    ...
                    ret.nonUpdatingInsert(member: ExplodingGem(index: currentIndex, type: board[currentIndex.x][currentIndex.y].type, cause: .SuperGemLine(index: explosion.index)))
                    clearGemAt(index: currentIndex)
                }
                for j in 2..<(dimension + 2) {
        ...
        for i in 2..<(dimension + 2) {
            for j in 2..<(dimension + 2) {
                if board[i][j].type == type {
                    ret.nonUpdatingInsert(member: GemIndex(x: i, y: j))
                }
            }
        }
        return ret
        ...
        for i in 2..<(dimension + 2) {
            for j in 2..<(dimension + 2) {
                if !excluding.contains(GemIndex(x: i, y: j)) {
                    types.nonUpdatingInsert(member: board[i][j].type)
                }
            }
        }
    ...
    init(coder aDecoder: NSCoder) {
        dimension = aDecoder.decodeInteger(forKey: PropertyKey.dimensionKey)
        board = aDecoder.decodeObjectForKey(PropertyKey.boardKey) as! [[Gem]]
    }
    
    func encodeWithCoder(aCoder: NSCoder) {
        aCoder.encode(dimension, forKey: PropertyKey.dimensionKey)
        aCoder.encode(board, forKey: PropertyKey.boardKey)
    }
