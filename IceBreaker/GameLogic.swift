...
class GameLogic: BoardTouchDelegate, PresenterCallback {
    ...
    private func finalizeStartup(index1: GemIndex, index2: GemIndex) {
        gameScoreDelegate?.clearScore()
        state = .Step0(hintTile1: index1, hintTile2: index2, timeIsUp: false)
        boardDelegate?.resume()
        boardDelegate?.showBoard(board: board, animate: true)
    ...
    func tile(tile: GemIndex, swipeDirection direction: SwipeDirection) {
        ...
        if !tileIsOnBoard(tile: otherTile) {
            return
        }
    ...
    private func gameEnded(timeIsUp: Bool = true) {
        state = .Step4(timeIsUp: timeIsUp)
        if let scoreDelegate = gameScoreDelegate {
            gamePlayDelegate?.gameEnded(score: scoreDelegate.currentScore(), newHighScore: scoreDelegate.newHighScore(), board: board, timeIsUp: timeIsUp)
        } else {
            gamePlayDelegate?.gameEnded(score: 0, newHighScore: false, board: board, timeIsUp: timeIsUp)
        }
    }
    ...
    func animationDone() {
        ...
        case .Step1a(let tile, let otherTile, let timeIsUp, let gem1, let gem2):
            ...
            gameScoreDelegate?.addScore(score: countToScore(count: explosion.count))
            boardDelegate?.firstExplosion(explosion: explosion, onNewBoard: board, gem1: gem1, gem2: gem2)
        case .Step1b(let scoreMultiplier, let timeIsUp):
            ...
            gameScoreDelegate?.addScore(score: scoreMultiplier * countToScore(count: explosion.count))
            boardDelegate?.firstExplosion(explosion: explosion, onNewBoard: board, gem1: nil, gem2: nil)
        ...
        case .Step2(let explosion, let scoreMultiplier, let timeIsUp):
            ...
            gameScoreDelegate?.addScore(score: scoreMultiplier * countToScore(count: nextExplosion.count))
            state = .Step2(previousExplosion: nextExplosion, scoreMultiplier: scoreMultiplier + 1, timeIsUp: timeIsUp)
            boardDelegate?.nextExplosion(explosion: nextExplosion, previousExplosion: explosion, onNewBoard: board)
        case .Step3(let scoreMultiplier, let timeIsUp):
            ...
            boardDelegate?.fellGems(indexes: indexPairs, withNewGems: newGems)
        case .Step4:
            break
        }
    }
