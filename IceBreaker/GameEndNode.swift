...
class GameEndNode: SKSpriteNode, ResizableNode {
    ...
    func showScreen(score: Int, newHighScore: Bool, parentSize: CGSize) {
        ...
        tapLabel.alpha = 0
        let enableTouchAction = SKAction.sequence([SKAction.fadeIn(withDuration: 3 * UIGlobals.gameSpeed), SKAction.run({[weak self] in self?.touchEnabled = true})])
        tapLabel.run(enableTouchAction)
    }
    ...
    func randomizeGem() {
        ...
        for (count, item) in GameEndNode.gemTextures.enumerated() {
            if count == dice {
                gem.texture = item.1
            }
        }
